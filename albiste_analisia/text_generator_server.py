...
def textu_generatzailea_servers():
    ...
    with col2:
        ...
        client = OpenAI(
            base_url=conf_text.api.api_url,
            api_key=conf_text.api.api_key,
        )
        def generate_field(sys_prompt, albistea, client):
 
            completion = client.chat.completions.create(
                model="HiTZ/Latxa-Llama-3.1-8B-Instruct:featherless-ai",
                messages=[
            ...
            for campo, valor, icono, sys_prompt, max_tokens in [
                ...
                with col7:
                  if st.button("🔄 Berriz sortu", key=f"{campo}_regen"):
                        new_value = generate_field(sys_prompt, albistea,client)
